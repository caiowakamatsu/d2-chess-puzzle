...
def process_image_into_cells(image, ctx, debug, debug_internal, debug_cells):
    cells = get_cells(image, ctx, debug=debug, debug_internal=debug_internal)
    processed_cells = []
    for cell in cells:
        processed_cells.append(normalize_cell(cell, ctx=ctx, debug=debug_cells))
